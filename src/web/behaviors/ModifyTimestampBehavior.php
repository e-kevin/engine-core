 ...
 *  - 1、在[[ActiveRecord::rules()]]方法里使用[[createRules()]]方法返回结果，行为会根据行为配置自动处理规则
 *  - 2、在[[ActiveRecord::scenarios()]]方法里使用[[createScenarios()]]方法返回结果，行为会根据行为配置自动处理场景
 * @example
 * ```php
...
class ModifyTimestampBehavior extends TimestampBehavior
{
     ...
     */
    public $dateTimeWidget = '\EngineCore\widgets\DateControl';
    ...
    protected function addRules(&$rules, $attribute)
    {
        ...
        switch ($this->dateTimeWidget) {
            case '\EngineCore\widgets\DateTimePicker':
                // '1901-12-14 04:45:52'的时间戳为 -2147483648，
                //mysql int类型保存的范围从 -2^31 (-2,147,483,648) 到 2^31 – 1 (2,147,483,647) 的整型数据（所有数字）。
                //存储大小为 4 个字节。
                // 添加时间范围限制
                ...
                break;
            case '\EngineCore\widgets\DateControl':
            case '\kartik\datecontrol\DateControl':
                switch ($saveData['format']) {
                    case 'php:U':
                        $rules[] = [$attribute, '\EngineCore\validators\DateControlValidator'];
                        break;
                    default:
                        // 添加时间范围限制
                        $rules[] = [$attribute, $displayData['type'],
