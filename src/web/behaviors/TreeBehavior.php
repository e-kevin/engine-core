...
use Yii;
use yii\{
    base\Behavior, base\Component, base\ModelEvent
};
...
class TreeBehavior extends Behavior
{
    ...
    private function _formatTree($list, $level = 0)
    {
        $formatTree = [];
        foreach ($list as $val) {
            $tmpStr = str_repeat($this->tabSize, $level);
            $tmpStr .= "└&nbsp;";
            $val['level'] = $val['level'] ?? intval($level);
            $val[$this->showTitleField] = ($val['level'] ? $tmpStr : '') . $val[$this->showTitleField];
            if (!array_key_exists('items', $val)) {
                ...
                unset($val['items']);
                array_push($formatTree, $val);
                $formatTree = array_merge($formatTree, $this->_formatTree($tmpAry, $level + 1));
            }
        }
